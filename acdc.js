...
// Runtime vars
acdc.runInBrowser = true; // false if run in shell
 ...
 * @param {number} isBrowserOutput
 * @returns {Output}
 * @constructor
 */
function Output (isBrowserOutput) {
    this.isBrowser_ = isBrowserOutput;
}
Output.prototype.println = function(message, id) {
    if (this.isBrowser_) {
        var console = document.getElementById('browser_console');
...
function setupEnvironment() {
    acdc.out = new Output(acdc.runInBrowser);
}

function checkBrowser() {
    // TODO: check for a better way to do this
    if (typeof window === 'undefined') {
        acdc.runInBrowser = false;
    }
}
...
if (typeof(arguments) !== 'undefined') {
    cliArgs = Array.prototype.slice.call(arguments);
}
if (typeof(scriptArgs) !== 'undefined') {
    cliArgs = Array.prototype.slice.call(scriptArgs);
}
